...
<head>
    ...
    <title>{{ time() . '-venta-' . $sale->n_comprobante }}</title>
    <style>
        body{
...
<body>
    <h1>Ingreso N° {{ $sale->n_comprobante }}</h1>
                                    ...
                                    <tbody>
                                        @foreach ($detalles as $d)
                                            <tr>
                                                ...
                                                <td>${{ $d->sale_price }}</td>
                                                <td>${{ $d->discount }}</td>
                                                <td>${{ $d->quantity * $d->sale_price - $d->discount }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                    <tfoot style="border: 1px solid cadetblue">
                                        ...
                                        <th>
                                            @php
                                                $total = 0;
                                                foreach ($detalles as $d) {
                                                    $total += $d->quantity * $d->sale_price - $d->discount;
                                                }
                                            @endphp
